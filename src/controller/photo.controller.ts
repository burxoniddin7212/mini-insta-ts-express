import { PhotoRepository, photoSchema } from './../repositories/photo.repositories';
...
let UPDATE = async (req: DemoRequest, res: Response) => {
   try {
      ...
      let record = await PhotoRepo.photoUpdate(req.body, req.user_id as string, req.file?.filename as string)
...
let DELETE = async (req: DemoRequest, res: Response) => {
   try {
      ...
      let record = await PhotoRepo.photoDelete({ user_id: req.user_id as string, _id: req.params.id })
